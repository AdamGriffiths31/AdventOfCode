...
func strength(hand []Hand) {
	...
	total := 0
	for i, h := range hand {
		fmt.Printf("%s %d %d\n", h.originalCards, h.bid, h.classification)
		total += (i + 1) * h.bid
	}
	fmt.Println(total)
}
...
func classify(hand Hand) int {
	replacementHands := replacements(hand.originalCards)
	maxScore := 0

	for _, replacement := range replacementHands {
		currentScore := score(replacement)
...
func replacements(hand string) []string {
	if hand == "" {
		return []string{""}
	}
	...
	if firstCard == "J" {
		firstCard = "23456789TQKA"
	}

	for _, x := range firstCard {
		for _, y := range replacements(hand[1:]) {
